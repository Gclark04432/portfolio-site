'use client';

import {
  CodeBracketIcon,
  MagnifyingGlassCircleIcon,
} from '@heroicons/react/24/solid';
import {
  CSSLogo,
  HTMLLogo,
  JavascriptLogo,
  NextjsWhiteLogo,
  ReactLogo,
  TailwindcssLogo,
  TypescriptLogo,
} from '../../public/logos';

import { motion } from 'framer-motion';
import { useEffect, useState } from 'react';

export const Home = () => {
  const [isVisible, setIsVisible] = useState<boolean>(false);

  useEffect(() => {
    setTimeout(() => {
      setIsVisible(true);
    }, 500);
  }, []);

  return (
    <>
      <section
        id='home'
        className='flex h-screen w-screen flex-col justify-center justify-items-center bg-stone-950 px-10 py-30'
      >
        <div className='flex w-full self-center md:w-2xl lg:w-4xl'>
          <div className='flex w-4/6 flex-wrap justify-start self-center'>
            <div>
              <div className='flex w-full text-4xl font-extrabold'>
                <div className='text-white'>Hi! I'm</div>
                <div className='ml-1 text-rose-700'>Gary</div>
                <div className='text-white'>.</div>
              </div>
              <div className='mt-10 w-full text-xl font-medium text-gray-400'>
                I'm a software engineer specializing in frontend development and
                web design.
              </div>
            </div>
          </div>
          <div className='flex w-2/6 items-center justify-end'>
            <div className='shrink-0 items-center'>
              <img
                alt='Gary Clark'
                src='/images/profile.png'
                className='size-36 h-auto rounded-xl border border-rose-700 grayscale-50'
              />
            </div>
          </div>
        </div>
        <div className='flex flex-col self-center text-white md:w-2xl lg:w-4xl'>
          <div className='mb-2 flex font-bold text-white'>
            <CodeBracketIcon className='mr-2 w-4 text-red-700' />
            Core Skills:
          </div>
          {isVisible && (
            <div className='flex h-10 flex-wrap'>
              <motion.div
                initial={{ opacity: 0, x: -80 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.5 }}
                className='items-center'
              >
                <div className='mr-2 flex'>
                  <TypescriptLogo />
                  <div className='ml-1'>Typescript</div>
                </div>
              </motion.div>
              <motion.div
                initial={{ opacity: 0, x: -80 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 1 }}
                className='items-center'
              >
                <div className='mr-2 flex items-center'>
                  <JavascriptLogo />
                  <div className='ml-1'>Javascript</div>
                </div>
              </motion.div>
              <motion.div
                initial={{ opacity: 0, x: -80 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 1.5 }}
                className='items-center'
              >
                <div className='mr-2 flex items-center'>
                  <ReactLogo />
                  <div className='ml-1'>React</div>
                </div>
              </motion.div>

              <motion.div
                initial={{ opacity: 0, x: -80 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 2 }}
                className='items-center'
              >
                <div className='mr-2 flex items-center'>
                  <NextjsWhiteLogo />
                  <div className='ml-1'>Next.js</div>
                </div>
              </motion.div>

              <motion.div
                initial={{ opacity: 0, x: -80 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 2.5 }}
                className='items-center'
              >
                <div className='mr-2 flex items-center'>
                  <HTMLLogo />
                  <div className='ml-1'>HTML</div>
                </div>
              </motion.div>

              <motion.div
                initial={{ opacity: 0, x: -80 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 3 }}
                className='items-center'
              >
                <div className='mr-2 flex items-center'>
                  <CSSLogo />
                  <div className='ml-1'>CSS</div>
                </div>
              </motion.div>

              <motion.div
                initial={{ opacity: 0, x: -80 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 3.5 }}
                className='items-center'
              >
                <div className='mr-2 flex items-center'>
                  <TailwindcssLogo />
                  <div className='ml-1'>Tailwind CSS</div>
                </div>
              </motion.div>
            </div>
          )}
        </div>
        <div
          className='flex w-full self-center text-white md:w-2xl lg:w-4xl'
          style={{ marginTop: isVisible ? '10px' : '50px' }}
        >
          <MagnifyingGlassCircleIcon className='mr-2 w-4 text-red-700' />
          Open For Work
        </div>
      </section>
    </>
  );
};
